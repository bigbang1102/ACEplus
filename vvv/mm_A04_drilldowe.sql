...
IF
	( tsodetail.detailorderno = 100, tso.txno, '' ) AS txno,
IF
	...
	ifnull( ( tso.movementtypename ), '' ) AS movementtype,
...
FROM view_mm_tso_header AS tso
	...
	LEFT JOIN ( SELECT tso.id, ob.txdate FROM transferstocktransferstock AS tso LEFT JOIN outbounddeliveryoutbounddelivery AS ob ON tso.flow_refdocno = ob.txno ) AS ob ON tso.id = ob.id
	LEFT JOIN ( SELECT tso.id, tsr.txdate FROM transferstocktransferstock AS tso LEFT JOIN transferstockrequesttransferstockrequest AS tsr ON tso.flow_refdocno = tsr.txno ) AS tsr ON tso.id = tsr.id 
WHERE
	1 = 1 $P!{ExternalWhereClause} 
ORDER BY
	tso.txno,
	tsodetail.detailorderno
...
(
    ...
    SELECT 
        ...
        /* FG / DE / LO QUANTITY */
        IF(issue.storagelocationcode = $P{storagestatus},  SUM(issue.quantity), 0) AS FG_actual_quantity,
        IF(issue.storagelocationcode = $P{storagestatus2}, SUM(issue.quantity), 0) AS DE_actual_quantity,
        IF(issue.storagelocationcode <> $P{storagestatus}
           AND issue.storagelocationcode <> $P{storagestatus2}, SUM(issue.quantity), 0) AS LO_actual_quantity,

        IF(issue.storagelocationcode = $P{storagestatus},  SUM(issue.quantity2), 0) AS FG_actual_quantity2,
        IF(issue.storagelocationcode = $P{storagestatus2}, SUM(issue.quantity2), 0) AS DE_actual_quantity2,
        IF(issue.storagelocationcode <> $P{storagestatus}
           AND issue.storagelocationcode <> $P{storagestatus2}, SUM(issue.quantity2), 0) AS LO_actual_quantity2,

        IF(issue.storagelocationcode = $P{storagestatus},  SUM(issue.cost_amount), 0) AS FG_actual_cost_amount,
        IF(issue.storagelocationcode = $P{storagestatus2}, SUM(issue.cost_amount), 0) AS DE_actual_cost_amount,
        IF(issue.storagelocationcode <> $P{storagestatus}
           AND issue.storagelocationcode <> $P{storagestatus2}, SUM(issue.cost_amount), 0) AS LO_actual_cost_amount,
    ...
    SELECT 
        ...
        IF(receive.storagelocationcode = $P{storagestatus},  SUM(receive.quantity), 0) AS FG_actual_quantity,
        IF(receive.storagelocationcode = $P{storagestatus2}, SUM(receive.quantity), 0) AS DE_actual_quantity,
        IF(receive.storagelocationcode <> $P{storagestatus}
           AND receive.storagelocationcode <> $P{storagestatus2}, SUM(receive.quantity), 0) AS LO_actual_quantity,

        IF(receive.storagelocationcode = $P{storagestatus},  SUM(receive.quantity2), 0) AS FG_actual_quantity2,
        IF(receive.storagelocationcode = $P{storagestatus2}, SUM(receive.quantity2), 0) AS DE_actual_quantity2,
        IF(receive.storagelocationcode <> $P{storagestatus}
           AND receive.storagelocationcode <> $P{storagestatus2}, SUM(receive.quantity2), 0) AS LO_actual_quantity2,

        IF(receive.storagelocationcode = $P{storagestatus},  SUM(receive.cost_amount), 0) AS FG_actual_cost_amount,
        IF(receive.storagelocationcode = $P{storagestatus2}, SUM(receive.cost_amount), 0) AS DE_actual_cost_amount,
        IF(receive.storagelocationcode <> $P{storagestatus}
           AND receive.storagelocationcode <> $P{storagestatus2}, SUM(receive.cost_amount), 0) AS LO_actual_cost_amount,

...
(
    ...
    SELECT
        ...
        IFNULL(issue.materialtypecode,'')   AS plan_materialtypecode,
        IFNULL(issue.materialtypename,'')   AS plan_materialtypename,
        IFNULL(issue.materialcode,'')       AS plan_materialcode,
        IFNULL(issue.materialname,'')       AS plan_materialname,

        IFNULL(issue.plan_quantity,0)       AS plan_qty,
        IFNULL(issue.actual_quantity,0)     AS actual_qty,

        IFNULL(
            IF(issue.actual_cost_amount > 0 AND issue.actual_quantity > 0,
                issue.actual_cost_amount / issue.actual_quantity,
                0
            ),0
        ) AS actual_cost_per_unit,
        ...
        IF($P{sort_order1} IN ('materialtype','materialtypecode'),
                                              issue.materialtypename,
                                              ''
        )))))) AS sort_field1,
        ...
        IF($P{sort_order2} IN ('materialtype','materialtypecode'),
                                              issue.materialtypename,
                                              ''
        )))))) AS sort_field2
    ...
    WHERE
        ...
        AND IFNULL(issue.materialtypecode,'') <> ''
    ...
    SELECT
        ...
        receive.plan_quantity      AS fg_plan_qty,
        receive.actual_quantity    AS fg_actual_qty,

        0 AS actual_cost_per_unit,
        receive.actual_cost_amount AS fg_cost,

        ROUND((receive.actual_quantity / receive.plan_quantity) * 100,2) AS fg_percent,

        CONCAT(
            ROUND(receive.actual_detailorderno/100,0),
            '/',
            c_gr35.c_gr35
        ) AS fg_count,
        ...
        IF(
            DATEDIFF(
                gr.txdate,
                ADDDATE(wip.plan_txdate, CAST(wip.external_refdocno AS UNSIGNED))
            ) < 0,
            0,
            DATEDIFF(
                gr.txdate,
    ...
    FROM
        ...
        LEFT JOIN (
            SELECT
                a.id,
                COUNT(a.txno) AS c_gr35
            FROM
                workinprocessworkinprocess a
                INNER JOIN grgr b ON a.txno = b.flow_refdocno
            WHERE
                b.movementtypecode = 'GR-35'
            GROUP BY a.txno
        ) AS c_gr35 ON c_gr35.id = wip.id
    WHERE
        1 = 1
        $P!{ExternalWhereClause}

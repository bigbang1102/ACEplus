...
left join view_apap_form D on D.request_personid = C.id AND B.flow_refdocno = D.txno AND B.flow_refdoctype = D.txtype
WHERE A.txno= $P{txno}
...
apref.materialcode
, apref.materialname
, apref.quantity as qty
, apref.order_unitname as uom
, apref.price_per_unit as price_per_unit
, if(apref.discount_value ='',0.00000,apref.discount_value) as discount
, apref.netamount_after_vat as netamount_after_vat
, (apref.amount_before_previous_discount) as total
,IF(LENGTH(apref.materialcode)
<=41,CONCAT(SUBSTR(apref.materialcode,1,23),
" ",SUBSTR(apref.materialcode,24,17)),
CONCAT(SUBSTR(apref.materialcode,1,23),
" ",SUBSTR(apref.materialcode,24,17),"...")) as length_matcode
,SUBSTR(apref.materialcode,41) as length_matcode1
,IF((LENGTH(apref.materialname)+LENGTH(SUBSTR(apref.materialcode,41)))-6 <=85,
CONCAT(SUBSTR(apref.materialname,1,
(85-length(SUBSTR(apref.materialcode,41)))-4)),
CONCAT(SUBSTR(apref.materialname,1,
(85-length(SUBSTR(apref.materialcode,41)))-6),"...")) as length_matname
from view_pvpv_form pv
left join pvpvmaterialitem pvitem on pv.id = pvitem.pvid
left join view_apap_form ap on pvitem.flow_refdocno=ap.txno
left join view_apapmaterialitem_form apitem on ap.id = apitem.apid and pvitem.flow_refdetailorderno = apitem.detailorderno
left join view_apapreferenceitem_form apref on ap.id=apref.apid and apitem.detailorderno = apref.material_detailorderno
where pv.txno = $P{txno}
limit $P{row_display},1

-- pv_sub_product_1
select
apref.row_count as row_count
,apref.amount_total as amount_total
...
,round(round(apref.amount_total,2)-ifnull(pvdep.netamount_after_vat,0)-(pv.netamount_after_vat),2) as other_discount
...
left join (select pv.id,sum(round(apref.netamount_after_vat,2)) as amount_total,count(apref.detailorderno) as row_count,sum(apref.quantity) as quantity from view_pvpv_form pv
left join view_pvpvmaterialitem_form pvitem on pv.id = pvitem.pvid
left join view_apap_form ap on pvitem.flow_refdocno=ap.txno
left join view_apapmaterialitem_form apitem on ap.id = apitem.apid and pvitem.flow_refdetailorderno = apitem.detailorderno
left join view_apapreferenceitem_form apref on ap.id=apref.apid and apitem.detailorderno = apref.material_detailorderno
where pv.txno =  $P{txno}
) as apref on pv.id = apref.id
where pv.txno = $P{txno}

...
FROM
  ...
  LEFT JOIN ( SELECT ap.id, ib.txdate 
  FROM inbounddeliveryinbounddelivery AS ib 
  LEFT JOIN apap AS ap 
  ON ap.flow_refdocno = ib.txno ) AS ib 
  ON ap.id = ib.id 
  AND ap.FLOW_REFDOCTYPE IN ( 'ib', 'ib5' )
  LEFT JOIN ( SELECT ap.id, po.txdate 
  FROM popo AS po 
  LEFT JOIN apap AS ap 
  ON ap.flow_refdocno = po.txno ) AS po ON ap.id = po.id 

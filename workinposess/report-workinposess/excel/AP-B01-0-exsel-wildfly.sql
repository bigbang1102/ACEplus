SELECT
  ...
  sum(apdetail.settlement_amount) AS settle_amount,
...
FROM
  ...
  LEFT JOIN (SELECT ap.id, ib.txdate FROM view_pc_ib_header AS ib LEFT JOIN view_ap_ap1_header AS ap ON ap.flow_refdocno = ib.txno) AS ib ON ap.id = ib.id
  AND ap.FLOW_REFDOCTYPE IN ('ib', 'ib5')
  LEFT JOIN (SELECT ap.id, po.txdate FROM popo AS po LEFT JOIN view_ap_ap1_header AS ap ON ap.flow_refdocno = po.txno) AS po ON ap.id = po.id

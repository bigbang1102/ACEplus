SELECT 
    ...
    ap.prepaid_amount,
    ap.movementtypecode AS movcode,
    ap.movementtypename AS movname,
    ap.businesspartnercode AS vendorcode,
    ap.businesspartnername AS vendorname,
    ap.refdocno, 
    ap.status, 
    ap.remark,
    ap.branchcode,
    ...
    IF(ap.vat_type = 1,
        (apdetail.netamount_before_vat - 
            (apdetail.adjust_amount - 
                ROUND(apdetail.adjust_amount * 
                    ROUND(apdetail.vat_amount * 100 / apdetail.netamount_before_vat, 2) / 
    ...
    IF(ap.vat_type = 1,
        ...
        (apdetail.vat_amount - 
            ROUND(apdetail.adjust_amount * 
                ROUND(apdetail.vat_amount / apdetail.netamount_after_vat, 2), 5))) AS vat_amount,
    IF(ap.vat_type = 1,
        apdetail.netamount_after_vat - apdetail.adjust_amount,
        (apdetail.netamount_after_vat - 
            ROUND(apdetail.adjust_amount * 
                (1 + ROUND(apdetail.vat_amount / apdetail.netamount_after_vat, 2)), 5))) AS netamount,
        ...
        IF($P{sort_order1}='status', ap.status,
            IF($P{sort_order1}='movcode', ap.movementtypecode,
                IF($P{sort_order1}='txdate', DATE_FORMAT(ap.txdate, '%Y-%m-%d'),
                    IF(($P{sort_order1}='branchname' OR $P{sort_order1}='branchcode'), ap.branchname,
                        IF(($P{sort_order1}='vendorname' OR $P{sort_order1}='vendorcode'), ap.businesspartnername, '')))))) AS sort_field1,
        ...
        IF($P{sort_order2}='status', ap.status,
            IF($P{sort_order2}='movcode', ap.movementtypecode,
                IF($P{sort_order2}='txdate', DATE_FORMAT(ap.txdate, '%Y-%m-%d'),
                    IF(($P{sort_order2}='branchname' OR $P{sort_order2}='branchcode'), ap.branchname,
                        IF(($P{sort_order2}='vendorname' OR $P{sort_order2}='vendorcode'), ap.businesspartnername, '')))))) AS sort_field2

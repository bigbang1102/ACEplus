...
, 'companystock' as storage_code, 'companystock' as storage_name
, ifnull(stkstatus.code,'') as stockstatus_code
, ifnull(stkstatus.name,'') as stockstatus_name
, ifnull(batch.code,'') as batch_code, ifnull(batch.name,'') as batch_name
, (sum.open_qty) as unit_open
#,temp_sum.sum_unit_open as sum_unit_open
...
poststockcompanypoststockcompanysummary as sum
	 left join poststockcompanypoststockcompanymovement as mov on sum.material = mov.material and sum.stockstatus = mov.stockstatus and sum.batchnumber = mov.batchnumber and mov.txdate>=$P{filter_todate_before} and mov.txdate<=$P{filter_todate_after} and mov.status='P'
...
group by  mov.txno , sum.material ,sum.stockstatus
order by  sum.material ,mov.txdate,sum.stockstatus, sum.processyear,sum.processmonth,mov.txno